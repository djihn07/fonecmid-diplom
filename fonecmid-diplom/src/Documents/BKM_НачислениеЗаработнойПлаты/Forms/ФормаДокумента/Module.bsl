...
Процедура ОсновныеНачисленияСотрудникПриИзменении(Элемент)
	...
	Если ТекущаяСтрока <> Неопределено И ЗначениеЗаполнено(ТекущаяСтрока.Сотрудник) Тогда
		...
		НайденныйГрафик = ПолучитьГрафикСотрудникаНаСервере(ТекущаяСтрока.Сотрудник, Объект.Дата);
		
		Если НайденныйГрафик <> Неопределено Тогда
			ТекущаяСтрока.ГрафикРабот = НайденныйГрафик;
		КонецЕсли;
...
Функция ПолучитьГрафикСотрудникаНаСервере(ВыбранныйСотрудник, ДатаДокумента)
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	BKM_УсловияОплатыСотрудниковСрезПоследних.ГрафикРабот КАК ГрафикРабот
	|ИЗ
	|	РегистрСведений.BKM_УсловияОплатыСотрудников.СрезПоследних(&Период, Сотрудник = &Сотрудник) КАК BKM_УсловияОплатыСотрудниковСрезПоследних";
