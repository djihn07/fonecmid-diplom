...
#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	...
	Процедура ОбработкаПроведения(Отказ, Режим)
		СформироватьАвтоматическиеПремии(Отказ);
		Если Отказ Тогда 
			Возврат; 
		КонецЕсли; 
		
		Если  ОсновныеНачисления.Количество() = 0 Тогда
			ТекстСообщения = "Табличная часть документа пуста. Документ не проведен.";
	...
	Процедура СформироватьАвтоматическиеПремии(Отказ)
		...
		|	Остатки.СуммаКОплатеОстаток КАК СуммаВыполненныхРабот,
		|	ЕСТЬNULL(Условия.ПроцентОтРабот, 0) КАК ПроцентОтРабот
		|ИЗ
		|	РегистрНакопления.BKM_ВыполненныеСотрудникомРаботы.Остатки(&Дата, ) КАК Остатки
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.BKM_УсловияОплатыСотрудников.СрезПоследних(&Дата, ) КАК Условия
		|		ПО Остатки.Сотрудник = Условия.Сотрудник";
		...
		Пока Выборка.Следующий() Цикл
			...
			// Если процент равен 0, премию не начисляем (опционально)
			Если Выборка.ПроцентОтРабот = 0 Тогда
				Продолжить;
			КонецЕсли;
			
			// Добавляем строку в табличную часть "ДополнительныеНачисления"
			СтрокаТЧ = ДополнительныеНачисления.Добавить();
			СтрокаТЧ.Сотрудник = Выборка.Сотрудник;
			// Расчет суммы: Сумма выполненных работ * (Процент / 100)
			СтрокаТЧ.СуммаНачисления = Выборка.СуммаВыполненныхРабот * (Выборка.ПроцентОтРабот / 100);
	...
	Процедура СформироватьДвиженияПоОсновнымНачислениям() 
		...
		Пока Выборка.Следующий() Цикл 
			...
			// Пропускаем записи с нулевым или несуществующимим окладами
	...
	Процедура РасчитатьОклады() 
		...
		// Создаем Соответствие для поиска
		// Ключ: НомерСтроки (или Сотрудник + ВидРасчета)
		// Значение: Объект движения
		КартаДвижений = Новый Соответствие;
		Для Каждого Движение Из Движения.BKM_ОсновныеНачисления Цикл
			// Используем НомерСтроки как уникальный идентификатор в рамках документа
			КартаДвижений.Вставить(Движение.НомерСтроки, Движение);
		КонецЦикла;
		...
		Пока Выборка.Следующий() Цикл
			...
			// Получаем движение из карты по номеру строки
			Движение = КартаДвижений.Получить(Выборка.НомерСтроки);
			
			// Проверка: если движение не найдено, пропускаем
	...
	Процедура РассчитатьОтпускныеНачисления()
		...
		Запрос.УстановитьПараметр("Измерения", Новый Массив);
		Запрос.УстановитьПараметр("Регистратор", Ссылка);
	...
	Процедура РасчитатьНДФЛ()
		...
		Для Каждого Движение Из Движения.BKM_Удержания Цикл
			КартаДвижений.Вставить(Движение.НомерСтроки, Движение);
		КонецЦикла;
